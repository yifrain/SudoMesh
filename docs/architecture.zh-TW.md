...
**精確模式（第 14 節）同時消除了這兩者。**

---

## 14. 精確模式：確定性單一負責人 + 虛擬節點（M5）

首解搜尋幾乎無法平行；真正的收益在**窮舉**搜尋（統計所有解 / 驗證唯一性）。樸素的
工作竊取在非同步 gossip 下會重複工作。*精確模式*讓叢集**確定性、零重複、計數精確**——
以 `--exclusive` 選擇（`benchmark` 預設開啟）。

### 運作原理
* **每個任務唯一負責人。** 任務 key 依 XOR 距離對應到唯一負責人；某節點僅當自己是
  負責人時才執行該任務（[`_owns`/`_owner_of`](../src/swarmsolve/peer.py)）。
* **可靠路由（DHT `put`）。** OPEN_TASK 不再 gossip，而是**經 TCP 直接投遞給負責人**
  （ttl=0 不再轉發），因此不會遺失——[`_route_open_task`](../src/swarmsolve/peer.py)。
* **靜態分發。** 所有任務於開始時一次性播種（更細的前沿），關閉執行期工作竊取——這消除
  了「任務晚到 / 終止判定」問題（修復前實測 peers=2 僅得 29,982/45,475 個解，因某節點在
  任務到達前就閒置退出）。
* **虛擬節點（一致性雜湊）。** 僅少數節點時，少量 ID 把 160 位元空間分割得極不均（某節點
  拿到約 47% 的工作）。於是每個節點註冊 `--vnodes` 個虛擬 ID；任務對應到最近的虛擬節點
  → 負載平衡。這正是第 5 章經典的負載平衡技術。

### 精確 vs 工作竊取

| 模式 | 重複工作 | 解計數 | 擅長 | 韌性 |
|------|----------|--------|------|------|
| 工作竊取（`--no-exclusive`） | 有（節點級） | 精確（dedup） | 不平衡樹 | 容忍遺失/抖動 |
| 精確（`--exclusive`，預設） | 無 | 精確 | 平衡樹 | 需要可靠投遞 |

這是一個乾淨的**一致性 vs 可用性**討論點：精確模式偏向精確/零重複；工作竊取偏向可用性/吞吐。

### 實測結果（用於報告）
在一個較寬的 9×9 上窮舉統計所有解（97,605 節點，45,475 個解），`--node-delay 0.0002`，
精確模式 + 16 虛擬節點，單機基準 ≈ 28–30 秒：

| 節點數 | 牆鐘時間 | vs 基準 | vs 單節點 | 各節點節點數分佈 |
|--------|----------|---------|-----------|------------------|
| 1 | 34.8 s | 0.80× | 1.0× | — |
| 2 | 20.3 s | 1.50× | 1.71× | 平衡 |
...
_三條曲線（按宣告順序）：**理想線性**（1/2/4×）·**實測 vs 單節點**（1.0/1.71/3.0×）·
**實測 vs 單機基準**（0.8/1.5/2.55×）。vs-單節點曲線緊貼理想（4 節點 75% 效率）；
vs-基準曲線偏低僅因每行程的框架固定開銷。_

每次執行都回報 **correctness OK：全部 45,475 個解恰好覆蓋一次**（節點總數 97,260–97,512
≈ 基準 97,605 → 約 0% 重複）。與理想 4× 的差距來自框架固定開銷（行程啟動 + settle +
閒置尾巴），表現為單節點的 0.80×；*相對單節點*，4 節點 scaling 約 3.0×（75% 效率）。重現：
```bash
...
### 25×25
求解器與尺寸無關（N = k²）。`swarmsolve gen --size 25` 瞬時建構合法 25×25；高線索實例靠
約束傳播立即解出（`solve` → 1 節點，0.00 秒）。稀疏線索的 25×25 首解搜尋是 NP 難（樹爆炸），
因此我們用上面受控的*窮舉*基準來量化平行加速，而非單次 25×25 首解執行。

---

## 15. 工作竊取：基於工作捐贈的動態負載平衡（M5+）

之前叢集只有*被動*切分（`split_depth` 廣播子任務）和*靜態*歸屬（exclusive 模式），
缺少**真正的工作竊取**：閒置節點主動向忙節點索要工作。現已實作。
...
閒置節點                            忙節點（持有在跑任務）
   ...
   |                                      |   退役父任務（mark_done）
   |                                      |   本地保留 0 號子任務
   |<---- WORK_DONATE (子任務) -----------|   回傳 N 號子任務
   |                                      |
納入 open 池                              在 0 號子任務上繼續
```
新增兩個訊息型別 [`WORK_REQUEST`](../src/swarmsolve/transport/messages.py) /
[`WORK_DONATE`](../src/swarmsolve/transport/messages.py)，**經 TCP 直接傳輸**
（不走 gossip）——它們是點對點協調。見
[`_on_work_steal`](../src/swarmsolve/peer.py) / [`_try_donate`](../src/swarmsolve/peer.py)
/ [`_maybe_steal`](../src/swarmsolve/peer.py)。

### 為何要退役父任務？
樸素捐贈（交出一個子任務但繼續探索父任務）會導致父與子**重疊** → 重複工作。
`_try_donate` 改為退役父任務（`mark_done`）並只保留一個子任務，使父+子任務
無重疊地劃分整棵子樹。

### 租約續約（防止長任務被重複）
長 DFS 可能超過租約時長而被其他節點回收重做。現在
[`_tick_and_should_stop`](../src/swarmsolve/peer.py)（每個搜尋節點呼叫一次）
會透過 [`Scheduler.renew`](../src/swarmsolve/tasks/scheduler.py) 續約所有在跑任務
的租約——忙節點不會被誤判為死節點。

### 兩種負載平衡策略及適用場景
| 策略 | 旗標 | 重複工作 | 計數 | 適用 |
|------|------|----------|------|------|
| 精確（靜態） | `--exclusive`（預設） | 無 | 精確 | 平衡樹 |
| 工作竊取（動態） | `--work-stealing` | 有（重疊） | 精確（dedup） | 不平衡樹、抖動 |

當搜尋樹**嚴重傾斜**（某分支遠大於其他）、節點否則會閒置時，工作竊取是正解：
忙節點在執行時把工作讓給閒置節點。樹均勻且要零重複時，精確模式更好。重現：
```bash
